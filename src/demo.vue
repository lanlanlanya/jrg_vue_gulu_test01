    ...
    <div>
        <div style="margin: 20px;">
            <g-table  :data-source="dataSource" bordered :selected-items.sync="selected" :order-by.sync="orderBy"
                ...
                <!--</template>-->
                <g-table-colum text="姓名" field="name" :width="100">
                    <template slot-scope="props">
                        <a href="http://baidu.com">{{props.value}}</a>
                    </template>
                </g-table-colum>
                <g-table-colum text="分数" field="score" ></g-table-colum>
            </g-table>
        </div>
        <div style="margin: 20px;">
...
<script>
    ...
    import GTableColum from './table-cloum'
    export default {
        name: "demo",
        components: {GPager, GTable,GTableColum},
        data () {
            return {
                currentPage: 1,
                selected: [],
